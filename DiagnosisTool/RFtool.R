invisible(lapply(c("data.table", "randomForest", "parallel"),function(pk){
...
save(Threshold_detect, file = "For_Rerun")
#####
...
save(PI_bio_merge, file = "../Impact_Probabilities")



invisible(lapply(c("sf", "ggplot2"),function(pk){
  if(!pk %in% row.names(installed.packages())){install.packages(pk)}
  library(pk,character.only=T)}))
setwd("D:/Mes Donnees/Hymobio/DataBase_treatment/Seuils")

load("Impact_Probabilities")

load("../BD_Bio/AllStation_ArianeMarch2023")

AllStations <- AllStations[CdStation %in% PI_bio_merge$ID]
...
PI_bio_merge <- melt(PI_bio_merge, measure = c("B_DIA", "B_FISH", "B_INV"),value.name = "PI",
                variable.name = "Community")[, lastYear := min(Year), by = c("ID","Community")][Year == lastYear]

PI_bio_merge <- merge(PI_bio_merge, unique(AllStations[,.(CdStation, CoordX_L93, CoordY_L93)]), 
...
Plot_cat <- 
  ...
  geom_point(data = PI_bio_merge, aes(x = CoordX_L93, y = CoordY_L93, color = PI), size = 1) +
  scale_color_gradient2(low = "cornflowerblue", mid = "snow2",high = "firebrick", na.value = NA, midpoint = 0.5,
                        name = "Impact probability", breaks = c(0,0.5,1), limits = c(0,1)) + 
  facet_wrap(Community ~ ., ncol = 3) + theme_void()
...
Plot_All <-
  ...
  ggplot(data = FrenchOutline) +  
  geom_sf( fill="white", color="gray") +
  geom_point(data = PI_bio_merge[, PI := mean(PI, na.rm = T), by = c("ID", "Community")], aes(x = CoordX_L93, y = CoordY_L93, colour = PI), size = 1) +
  scale_color_gradient(low = "cornflowerblue",high = "firebrick", name = "Biological state\n",
                        breaks = c(min(PI_bio_merge$PI, na.rm = T),max(PI_bio_merge$PI, na.rm = T)), 
