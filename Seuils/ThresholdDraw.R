...
Threshold_plot <- lapply(files, function(param){
  ...
  if(any(unlist(lapply(Thresh_draw, is.data.frame)))){
    ...
    if(length(Thresh_plot) > 0){
      ...
      Thresh_plot[, AlphaTest := "S"][AUC_valTest < 0.6, AlphaTest := "NS"][
        , AlphaTrain := "S"][AUC_valTrain < 0.6, AlphaTrain := "NS"]
      ...
      plot <- ggplot(data = Thresh_plot, aes(x = Threshold, y = AUC_valTest, color = CompartmentName, fill = CompartmentName)) + 
        geom_hline(yintercept = c(0.6, 0.7, 0.8), color = "darkgrey", lty = 1) + 
...
OtherMetrics_plot <- lapply(files, function(param){
  ...
  if(any(unlist(lapply(Thresh_draw, is.list)))){
    ...
    ifelse(which(Cat[Category == category, Names] == param) %in% 
          seq(min(3, length(Cat[Category == category, Names])), length(Cat[Category == category, Names]), by = min(4, length(Cat[Category == category, Names]))),
           IsCol <- "none", IsCol <- "none")#IsCol <- guide_legend(title.position="top", title.hjust = 0.5)
    ...
    ifelse(which(Cat[Category == category, Names] != param) %in% 
            ...
            IsShp <- "none", IsShp <- "none")#,IsShp <- guide_legend(title.position="top", title.hjust = 0.5))
...
Table <- merge(Table, Titles[,.(Description, Name_Origine, Unit, Category)])

write.table(Table[order(-AUC_valTest,-F1),.(Community, Category, Description, 
                 Unit, Threshold, AUC_valTest, Accuracy, Sensitivity, Specificity,
                 Precision, Recall, F1)], file = "FinalTable.csv", sep = ";", row.names = F)
