#!/usr/bin/Rscript  
if(!"DMwR" %in% row.names(installed.packages())){remotes::install_github("cran/DMwR")}

invisible(lapply(c("data.table", "ggplot2", "stringr", "ggpubr", "randomForest", "ROCR", 
                   "doParallel", "dplyr","caret", "DMwR"),function(pk){
...
for(param in Params){
    ...
    Thresh_AUC <- lapply(c("B_FISH", "B_INV","B_DIA"), function(comp){
      ...
      if(!any(c(nrow(Rfd_comp[State == "good"]), nrow(Rfd_comp[State == "bad"])) < 50) & uniqueN(Rfd_comp$State) > 1){
        ...
        AUC_10Cross <- lapply(1:10, function(cross){
          ...
          Cross_train <- Cross_train[, grep(paste0(comp,"|State"), colnames(Rfd_comp), value = T), with = F][
            , grep(comp, colnames(Rfd_comp), value = T) := lapply(.SD, function(X){
             return((X - min(X, na.rm = T))/(max(X, na.rm = T)-min(X, na.rm = T)))}), .SDcols = grep(comp, colnames(Rfd_comp), value = T)]
          
          Cross_train[is.na(Cross_train)] <- 0
          
          new_data <- rbind(Cross_train[State == Cross_train[,.N, by = "State"][which.max(N), State],],
                            SMOTE(State ~ ., Cross_train, perc.over = 200, perc.under = 0))
  ...
  save(Thresh_draw, file = paste0("AUC_threshold_2405_Q95_5stp_SMOTE_norm", param))
}
