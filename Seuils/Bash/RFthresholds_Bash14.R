...
for(param in Params){
  ...
  Thresh_draw <- lapply(Thresholds, function(Thresh){
     ...
     Thresh_AUC <- lapply(c("B_FISH", "B_INV","B_DIA"), function(comp){
      ...
      AUC_10CrossTest <- unlist(lapply(AUC_10Cross, function(cross){return(cross["Test"])}))
      AUC_10CrossTrain <- unlist(lapply(AUC_10Cross, function(cross){return(cross["Train"])}))
      
    return(data.frame(AUC_valTest = mean(AUC_10CrossTest, na.rm = T),
