...
Table <- do.call(rbind,lapply(files, function(param){
  ...
  Thresh_AUC[, DegradationDirection := Catalog[NameOrigin == gsub(toremove, "", param), DegradationDirection]]
    
    if(nrow(Thresh_AUC[AUC_valTest >= 0.7 & F1 >= 0.7]) > 0){
      ifelse(length(unique(Thresh_AUC[AUC_valTest >= 0.7 & F1 >= 0.7, CompartmentName])) > 1,
             Resp_community <- substr(unique(
               Thresh_AUC[AUC_valTest >= 0.7 & F1 >= 0.7 & !CompartmentName %in% Thresh_AUC[AUC_valTest == max(AUC_valTest),CompartmentName], CompartmentName]
             ), start=1, stop=4),
             Resp_community <- c("")
      )
      ...
      Main_comm <- unique(substr(Thresh_AUC[AUC_valTest == max(AUC_valTest), CompartmentName], start = 1, stop = 4))
...
Table <- Table[, (cols) := lapply(.SD, function(X) round(as.numeric(X), 2)),
               .SDcols = cols]
#setnames(Table, c("CompartmentName","Resp_comm"), c("Main_comm", "Responsive_comm"))
  ...
  if(any(unlist(lapply(Thresh_AUC, is.data.frame)))){
    ...
    # For AUC plot
      ...
      if(!is.na(Catalog[NameOrigin == gsub(toremove, "", param), LittThreshold]) && 
         ...
         !any(Thresh_AUC_max$THRmaxTest == as.numeric(Catalog[NameOrigin == gsub(toremove, "", param), LittThreshold]))){
        Thresh_AUC <- Thresh_AUC[Threshold != as.numeric(Catalog[NameOrigin == gsub(toremove, "", param), LittThreshold])]
      }
      ...
      Thresh_other[, CompartmentName := factor(CompartmentName, levels = c("Fish", "Macroinvertebrate", "Diatom"))][
      ...
      # Plot AUC
      plot_AUC <- ggplot(data = Thresh_AUC, aes(x = Threshold, y = AUC_valTest, color = CompartmentName, fill = CompartmentName)) + 
      ...
      # Plot other
      plot_other <- ggplot(data = Thresh_other, aes(x = Threshold, y = Med, color = CompartmentName, fill = CompartmentName)) + 
        geom_segment(data = Thresh_other_max, aes(x = THRintersect, xend = THRintersect, y = -0.05, yend = AccuIntersect), lty = 2, linewidth = 0.7) +
        theme_classic() + facet_grid(. ~ CompartmentName) + labs(x = "",y = Ytitle[3], color = "Community") + 
      ...
      ReturnTable <- merge(Thresh_AUC_max, Thresh_other_max)[, Thresh_diff := THRintersect - THRmaxTest][,
                     c("Rect_min", "Rect_max") := list(min(THRintersect, THRmaxTest), max(THRintersect, THRmaxTest)), by = CompartmentName]
...
#save(Threshold_plot, file = "ThresholdPlots_Final_2405")
#####
      ...
      Thresh_plot <- as.data.table(do.call(rbind,lapply(1:length(Thresh_plot), function(Xcomp) {
        ...
        if(length(Comp) !=0){
          Comp_Test <- as.data.table(do.call(rbind, lapply(1:length(Comp), function(xcomp){
            ...
            ret <- as.data.table(do.call(rbind, lapply(x, function(k) return(k$ConfusionTest$byClass))))
            ...
            return(unique(ret[,.(Compartment, Threshold, Metric, Low, Med, High)]))
          })
          ))
          Comp_Test <- merge(Comp_Test, data.table(Compartment = c("B_FISH", "B_INV", "B_DIA"),
                                                   CompartmentName = c("Fish", "Macroinvertebrate", "Diatom")), all.y = T)
