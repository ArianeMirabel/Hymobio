...
Inventory_TraitsFrequencies <- function(Code, Inventory, Traits) {
  ...
  Indices_Modalite <- lapply(unique(Code$Trait), function (trait) {
    ...
    ret <- lapply(Code[Trait == trait, Modalite], function(modalite){
      ...
      # (if the species doesn't have the modality, it's null), then sum by station and year
      # Nsp is the number of species 
      Inv[, Freq := Abundance/sum(Abundance), by = c("CdStation","Year")]
      
      Inv[, paste0("Abd_", modalite) := sum(Abundance * as.numeric(get(modalite))), by = c("CdStation","Year")][
        , paste0("Freq_", modalite) := sum(Freq * as.numeric(get(modalite))), by = c("CdStation","Year")][
          , paste0("Nsp_", modalite) := length(which(as.numeric(get(modalite)) != 0)), by = c("CdStation","Year")
        ]
      
      return(unique(Inv[,c("CdStation", "Year", paste0("Abd_", modalite), paste0("Freq_", modalite), paste0("Nsp_", modalite)), with = F]))
    })
...
#3 types of files: "Inventory" with operations, "Trait" with functional traits table, "Code" for functional traits decription and types
...
Modality_save <- lapply(c("Fish", "Macroinvertebrate", "Diatom"), function(compartment){
  ...
  rm(list = grep(compartment, ls(), value = T))
})