...
# Get Stations
#####
# Indiquer les colonnes à extraire
...
# Eliminer les stations hors france métropolitaine
...
# Fonction extraction permettant de continuer même si erreur réseau "Internal Server Error" )
...
# Debit Mensuel
#####
# Pour le mensuel pas besion de paralléliser, c'est rapide
# Creation de la table à remplir et d'un vecteur de stations n'ayant pas de données
...
for(station in unique(Hydro_Station$code_station)){
  ...
  # Les fichiers sont en plusieurs tranches, donc on compile les différentes tranches (on extrait tant que la tranche se termine par "next", indiquant qu'il y a une suite)
  if(extract$count != 0){
...
# Creation de la colonne année et enregistrement
...
# Debit Journalier
#####
# Pour le journalier on parralélise
...
#On va récupérer les fichiers enregistrés par tranche
